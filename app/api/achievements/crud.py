...
class CRUDAchievement(
    ...
    def create(
        ...
        max_achievements_per_day = 100 # TODO: change to 3
        ...
        receiver_citizen = db.query(citizen_models.Citizen).filter(citizen_models.Citizen.id == receiver_id).first()
        sender_citizen = db.query(citizen_models.Citizen).filter(citizen_models.Citizen.id == sender_id).first()
        
        if receiver_citizen and receiver_citizen.world_address:
            self.send_world_app_notification(db=db, reciever_adress=receiver_citizen.world_address, receiver=receiver_citizen, sender=sender_citizen)
        else:
            print("No citizen found or no world_address")
    ...
    def find(
        ...
        sent_achievements_with_citizens = db.query(self.model, citizen_models.Citizen).join(
            citizen_models.Citizen, 
            self.model.receiver_id == citizen_models.Citizen.id
        ).filter(self.model.sender_id == citizen_id).offset(skip).limit(limit).all()
        
        # Get received achievements with sender citizen data
        received_achievements_with_citizens = db.query(self.model, citizen_models.Citizen).join(
            citizen_models.Citizen, 
            self.model.sender_id == citizen_models.Citizen.id
        ).filter(self.model.receiver_id == citizen_id).offset(skip).limit(limit).all()
    ...
    def send_world_app_notification(
        ...
        print(f"Sending notification to {reciever_adress}")
        # Send a notification to the world app
